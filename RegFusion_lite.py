...
class AlignmentQualityEstimator(nn.Module):
    """对齐质量评估器，用于判断特征对齐的质量"""
    def __init__(self, feature_dim=256):
        super(AlignmentQualityEstimator, self).__init__()
        ...
        self.similarity_conv = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv2d(feature_dim, 64, 3, 1, 1),
            nn.ReLU(),
        ...
        # 光流一致性评估
        self.flow_consistency = nn.Sequential(
            nn.Conv2d(4, 16, 3, 1, 1),  # 2个光流场
            nn.ReLU(),
            nn.Conv2d(16, 1, 1, 1, 0),
            nn.Sigmoid()
        )
        
    def forward(self, f1, f2, flow1, flow2):
        """
        计算对齐质量分数
        Args:
            f1, f2: 对齐后的特征图 [B, C, H, W]
            flow1, flow2: 光流场 [B, 2, H, W]
        Returns:
            quality_score: 对齐质量分数 [B, 1, H, W]
        """
        # 计算特征相似度
        f_concat = torch.cat([f1, f2], dim=1)
        similarity = self.similarity_conv(f_concat)
        
        # 计算光流一致性
        flow_concat = torch.cat([flow1, flow2], dim=1)
        consistency = self.flow_consistency(flow_concat)
        
        # 综合质量分数
        quality_score = similarity * consistency
        return quality_score

class AdaptiveRegNet_lite(nn.Module):
    """带有自适应步数机制的RegNet_lite模块"""
    def __init__(self, max_steps=8, quality_threshold=0.8, min_steps=3):
        super(AdaptiveRegNet_lite, self).__init__()
        self.channels = [256, 64, 32, 16, 8, 1]
        self.max_steps = max_steps
        self.min_steps = min_steps
        self.quality_threshold = quality_threshold
        
        # 创建多个FusionRegBlk_lite实例，支持动态步数
        self.f1_blocks = nn.ModuleList()
        self.f2_blocks = nn.ModuleList()
        
        for i in range(self.max_steps):
            if i == 0:
                # 第一层
                self.f1_blocks.append(FusionRegBlk_lite(in_channel=self.channels[0], out_channel=self.channels[1]))
                self.f2_blocks.append(FusionRegBlk_lite(in_channel=self.channels[0], out_channel=self.channels[1]))
            elif i < len(self.channels) - 1:
                # 中间层
                self.f1_blocks.append(FusionRegBlk_lite(in_channel=self.channels[i], out_channel=self.channels[i+1]))
                self.f2_blocks.append(FusionRegBlk_lite(in_channel=self.channels[i], out_channel=self.channels[i+1]))
            else:
                # 最后一层重复使用
                self.f1_blocks.append(FusionRegBlk_lite(in_channel=self.channels[-2], out_channel=self.channels[-1]))
                self.f2_blocks.append(FusionRegBlk_lite(in_channel=self.channels[-2], out_channel=self.channels[-1]))
        
        self.D = Decoder(self.channels)
        
        # 对齐质量评估器
        self.quality_estimator = AlignmentQualityEstimator(feature_dim=256)
        
        # 步数决策网络
        self.step_decision = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Flatten(),
            nn.Linear(256 * 2 + 1, 64),  # 特征 + 质量分数
            nn.ReLU(),
            nn.Linear(64, 32),
            nn.ReLU(),
            nn.Linear(32, 1),
    ...
    def forward(self, f1, f2):
        ...
        for step in range(self.max_steps):
            ...
            # 特征融合和光流预测
            f_cat = torch.cat((current_f1, current_f2), dim=1)
            
            # 使用对应的FusionRegBlk
            f1_out, flow1 = self.f1_blocks[step](current_f1, f_cat)
            f2_out, flow2 = self.f2_blocks[step](current_f2, f_cat)
            
            # 应用光流进行特征对齐
            aligned_f1 = reg(flow1, current_f1)
            aligned_f2 = reg(flow2, current_f2)
            
            # 上采样特征（除了最后一步）
            if step < len(self.channels) - 1:
                aligned_f1 = getattr(self.D, f'up{step+1}')(aligned_f1)
                aligned_f2 = getattr(self.D, f'up{step+1}')(aligned_f2)
            
            # 存储输出
            flows.extend([flow1, flow2])
            f1_outputs.append(aligned_f1)
            f2_outputs.append(aligned_f2)
            
            # 计算对齐质量
            quality_score = self.quality_estimator(aligned_f1, aligned_f2, flow1, flow2)
            avg_quality = torch.mean(quality_score)
            
            # 准备决策输入
            decision_input = torch.cat([
                aligned_f1.mean(dim=[2, 3]),  # 全局平均池化
                aligned_f2.mean(dim=[2, 3]),
                avg_quality.unsqueeze(-1)
            ], dim=1)
            
            # 决定是否继续
            continue_prob = self.step_decision(decision_input)
            
            # 更新当前特征用于下一步
            current_f1 = aligned_f1
            current_f2 = aligned_f2
            
            # 自适应停止条件
            should_continue = (
                (step_count < self.min_steps) or  # 至少执行最小步数
                (avg_quality < self.quality_threshold and step_count < self.max_steps)  # 质量未达标且未达到最大步数
            )
            
            # 训练时使用网络决策，推理时使用阈值决策
            if self.training:
                should_continue = should_continue and (continue_prob > 0.5)
            else:
                should_continue = should_continue and (avg_quality < self.quality_threshold)
            
            if not should_continue:
                break
        
        # 使用最后一步的输出
        final_f1 = f1_outputs[-1]
        final_f2 = f2_outputs[-1]
        
        # 光流集成（使用实际产生的光流数量）
        actual_flows = flows[:step_count*2]  # 每步产生2个光流
        
        # 如果光流数量少于10个，用零填充
        while len(actual_flows) < 10:
            actual_flows.append(torch.zeros_like(actual_flows[0]))
        
        flow, flow_neg, flow_pos = flow_integration_ir(*actual_flows)
        
        return final_f1, final_f2, actual_flows, flow, flow_neg, flow_pos, step_count
